...
| 駅データ | N02（全駅） | **S12（乗降客数100人/日以上でフィルタリング）** |
| バス停データ | N07 | **P11（都道府県別最新）** |
| メッシュ解像度 | 250mメッシュ or 500mメッシュ | **250mメッシュ固定** |

道路ネットワーク距離により川・山で隔てられた「直線は近いが実際には遠い」ケースを正確に捕捉。S12乗降客数フィルタで実質廃線相当の駅を除外し、名目上の「便利地域」を減らせる。
...
| カテゴリ | メッシュ数 | 人口 | 割合 | 65歳以上人口 |
|---|---|---|---|---|
| 公共交通便利地域 | 110,206 | 4,330万人 | 34.4% | 1,047万人 |
| 公共交通不便地域 | 664,639 | 6,161万人 | 48.9% | 1,810万人 |
| **公共交通空白地域** | **5,160,282** | **2,103万人** | **16.7%** | **669万人** |
| 合計 | 5,935,127 | 1億2,594万人 | 100% | 3,526万人 |
...
**処理**（S12の場合）:
- LineString ジオメトリを EPSG:6677 で重心変換してポイント化
...
**定数**:
```python
MIN_PASSENGERS_PER_DAY = 100   # 変更可
S12_PASSENGER_COL      = "S12_061"  # 2024年
```

**出力**:

`data/stations.parquet`（6,572件）:
