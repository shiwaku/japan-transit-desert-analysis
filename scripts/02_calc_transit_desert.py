...
# 駅スナップ半径デフォルト値（m）: --station-snap-radius で上書き可能
# 通常は 0（LINE両端点スナップで複数出口に対応済みのため半径不要）
# 半径を指定すると各端点周辺の全道路ノードを追加スナップする
STATION_SNAP_RADIUS_M_DEFAULT = 0
...
def snap_to_nodes(coords: np.ndarray, node_coords: np.ndarray,
                  ...
                  radius_deg: float | None = None) -> np.ndarray:
    """
    施設（駅/バス停）座標を道路ノードにスナップし、グラフインデックスを返す。
    radius_deg 指定時: 半径内の全道路ノードをスナップ（北口・南口等の複数出口に対応）
    None 時: 最近傍1ノードのみ（バス停用）
    n2i に存在しないノードは除外。
    """
    tree = KDTree(node_coords)
    if radius_deg is not None:
        candidate_lists = tree.query_ball_point(coords, r=radius_deg)
        flat_idxs = []
        for i, candidates in enumerate(candidate_lists):
            if candidates:
                flat_idxs.extend(candidates)
            else:
                _, nearest = tree.query([coords[i]])
                flat_idxs.append(int(nearest[0]))
        snap_nids = nodes_df.iloc[flat_idxs]["node_id"].astype(int).to_numpy()
    else:
...
def main(station_snap_radius_m: float = STATION_SNAP_RADIUS_M_DEFAULT):
    t0 = time.time()
    station_snap_radius_deg = _m_to_deg(station_snap_radius_m) if station_snap_radius_m > 0 else None
    if station_snap_radius_deg:
        print(f"駅スナップ半径: {station_snap_radius_m:.0f}m  （≈{station_snap_radius_deg:.4f}°）")
    else:
        print("駅スナップ: LINE両端点 → 最近傍1道路ノード（半径なし）")
    ...
    st_idxs    = snap_to_nodes(st_coords, node_coords, nodes, n2i,
                               radius_deg=station_snap_radius_deg)
    ...
    bs_idxs    = snap_to_nodes(bs_coords, node_coords, nodes, n2i)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--station-snap-radius", type=float,
        default=STATION_SNAP_RADIUS_M_DEFAULT,
        metavar="METERS",
        help=f"駅スナップ半径（m）。北口・南口等の複数出口に対応（デフォルト: {STATION_SNAP_RADIUS_M_DEFAULT}m）",
    )
    args = parser.parse_args()
    main(station_snap_radius_m=args.station_snap_radius)
