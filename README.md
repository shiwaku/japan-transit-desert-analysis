...
| カテゴリ | 人口 | 割合 |
|---|---|---|
| 公共交通便利地域（鉄道駅 walk 1km以内） | **5,389万人** | 42.8% |
| 公共交通不便地域（バス停 walk 500m以内） | **5,358万人** | 42.5% |
| **公共交通空白地域** | **1,847万人** | **14.7%** |

**日本人の約7人に1人（1,847万人）が公共交通空白地域に住んでいる。**
...
| **本試算** | **約1,847万人** | **14.7%** | **道路距離 Dijkstra（令和2年国勢調査）** |

> 出典: 国交省リストアップ調査「『交通空白』リストアップ調査結果（概要）」（R7.4.30集計、回答率92%）

### 国交省 QGIS手順との差異：道路距離 vs 直線距離

本試算（1,847万人）が国交省QGIS手順の公表値（735万人）の**約2.5倍**になっている主因は距離計算方法の違いです。
...
道路が川・山・住宅地を迂回する場合、ネットワーク距離は直線距離の **1.2〜1.5倍** になることが多い。直線で800〜900m圏内の地域でも、実際の歩行距離は1km超となり「空白地域」に分類される。これが空白人口の乖離（約1,112万人分）の主要因です。

なお、国交省のリストアップ調査（約1,408万人）は自治体の自己申告ベースであり、GISによる距離計算とは手法が異なる。本試算の1,847万人とオーダーが近い点は、道路距離ベースの推計の妥当性を裏付けると考えられる。

> ※ QGIS手順の数値は「国土数値情報とQGISを活用した交通空白地の抽出と人口分析」（2025年4月）より。直線距離バッファによる簡易推計値。
...
| 道路ノード数 | 18,614,424件 |
| L5アクセスリンク数（250mメッシュ） | 5,935,127件 |
| 対象メッシュ（全国L5・250m） | 5,935,127メッシュ |
