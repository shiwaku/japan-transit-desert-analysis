...
> **駅スナップ方式**: S12ホーム線形の両端点を NE/NW/SE/SW 4象限スナップで道路ノードに紐付け（最大500m）。
> 線路をまたいだ東西口・南北口を確実にカバーする。1端点あたり最大4ノード採用。

| カテゴリ | 人口 | 割合 |
|---|---|---|
| 公共交通便利地域（鉄道駅 walk 1km以内） | **5,469万人** | 43.4% |
| 公共交通不便地域（バス停 walk 500m以内） | **4,730万人** | 37.6% |
| **公共交通空白地域** | **2,395万人** | **19.0%** |

**日本人の約5人に1人（2,395万人）が公共交通空白地域に住んでいる。**
...
本試算の空白地域人口（2,395万人）が国交省の公表値（735万人）の**約3.3倍**になっている主因は距離計算方法の違いです。
...
道路が川・山・住宅地を迂回する場合、ネットワーク距離は直線距離の **1.2〜1.5倍** になることが多い。直線で800〜900m圏内の地域でも、実際の歩行距離は1km超となり「空白地域」に分類される。これが空白人口の乖離（約1,069万人分）の主要因です。
...
- 各端点を NE/NW/SE/SW 4象限スナップで道路ノードに紐付け（スナップ駅ノード数: 66,269）
- 乗降客数フィルタリングなし（全駅使用）
...
駅スナップにはS12ホーム線形の両端点を使用し、各端点から NE/NW/SE/SW 4象限の最近傍ノードをスナップする。`--station-max-dist`（単位: m）で象限探索の最大距離を変更可能（デフォルト500m）。
