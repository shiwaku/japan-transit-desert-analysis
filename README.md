...
| カテゴリ | 人口 | 割合 |
|---|---|---|
| 公共交通便利地域（鉄道駅 walk 1km以内） | **4,330万人** | 34.4% |
| 公共交通不便地域（バス停 walk 500m以内） | **6,161万人** | 48.9% |
| **公共交通空白地域（車なしでは生活困難）** | **2,103万人** | **16.7%** |

**日本人の約6人に1人（2,103万人）が公共交通空白地域に住んでいる。**

65歳以上では **669万人**（高齢者全体の18.9%）が空白地域に居住。

## ウェブビューワー

`docs/index.html` — MapLibre GL JS + PMTiles による全国インタラクティブマップ。GitHub Pages で公開中。

- 住所検索で「あなたの実家」の交通区分を判定
...
| 駅データ | N02（全駅） | **S12（乗降客数100人/日以上）** |
| バス停データ | N07 | **P11（都道府県別最新）** |

道路ネットワーク距離により、川・山で隔てられた「直線は近いが実際には遠い」ケースを正確に捕捉できる。S12乗降客数フィルタで実質廃線相当の駅を除外し、名目上の「便利地域」を減らせる。
...
N02（鉄道路線）の代わりに S12 を使用。乗降客数（2024年）でフィルタリングすることで、廃線寸前・実質機能していない駅を除外できる。

- 乗降客数フィルタ閾値: **100人/日以上**（`01_prepare_facilities.py` の `MIN_PASSENGERS_PER_DAY` で変更可）
- S12のジオメトリは線型（ホーム線形）→ 重心をポイントとして使用
- 全国 10,534 駅 → フィルタ後 **6,572 駅**
...
# → data/stations.parquet（6,572駅・駅名・運営者・路線名付き）
...
Multi-source Dijkstra はスーパーソースノード方式で実装。駅6,572件・バス停242,985件をそれぞれ一括投入し、1回の探索で全メッシュへの最短徒歩時間を算出する。
